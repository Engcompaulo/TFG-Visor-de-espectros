...
class PreprocessOperator(BaseEstimator, TransformerMixin):
    ...
    def get_options(self):
        ...
        return {'normalize':{"type": ['l1', 'l2','cum','min','max','norm']},
...
class PreprocessPipeline(BaseEstimator, TransformerMixin):
    ...
    def __init__(self,att_names,orden,ori=50,end=1800,baseline='ALS_old',normalize="norm",squash="cos",smooth_type ='sg', smooth_size=25):
        ...
        for op in orden:    
            tipoOp_id[op]+=1
            if op=='C' and ori<end:
                steps.append(("Crop"+str(tipoOp_id[op]) ,PreprocessOperator("crop",feature_names=att_names,ori=ori, end=end)))

            if op=='B':
                steps.append(("Baseline"+str(tipoOp_id[op]),BaselineCorrection(baseline)))

            if op=='N':       
                steps.append(("Normalize"+str(tipoOp_id[op]) ,PreprocessOperator("normalize",type=normalize)))

            if op=='S':
                steps.append(("Squashing"+str(tipoOp_id[op]) ,PreprocessOperator("squash",type=squash)))

            if op=='s':
                steps.append(("Smoothing"+str(tipoOp_id[op]) ,PreprocessOperator("smooth",type=smooth_type,size=smooth_size)))

            if op=='d':
                steps.append(("Derive"+str(tipoOp_id[op]) ,PreprocessOperator("deriv")))
...
def preprocess_pipeline(X,att_names,orden,ori,end,baseline,normalize,squash,smooth_type,smooth_s):
    
    pp = PreprocessPipeline(att_names,orden,ori,end,baseline,normalize,squash,smooth_type,smooth_s)
    X = pp.transform(X)
