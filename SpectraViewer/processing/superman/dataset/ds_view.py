...
class DatasetView(object):
    ...
    def get_trajectories(self, return_keys=False):
        ...
        if hasattr(self.ds, 'intensities'):
            traj = self.ds.get_trajectories_by_index(self.mask,
    ...
    def get_vector_data(self):
        ...
        if not (crops and all(c[2] > 0 for c in crops)):
            raise ValueError(
                'Cannot create vector data from non-resampled trajs.')
    ...
    for i, k in enumerate(top_k):
        ...
        if num_endmembers == 1:
            ...
            if method == 'add':
                ...
                # recurse
                for s2, n2 in _cs_helper(sub, library, names,
                                         num_endmembers - 1,
                                         num_results, metric, num_procs,
                                         min_window,
                                         score_pct, method):
...
class MultiDatasetView(object):
    def __init__(self, ds_views):
        ...
        if len(kinds) != 1:
            raise ValueError(
                'Cannot use MultiDatasetView with >1 kind of dataset.')
        self.ds_kind, = kinds
    ...
    def get_primary_keys(self):
        if self.num_datasets == 1:
            return np.concatenate(
                [dv.get_primary_keys() for dv in self.ds_views])
    ...
    def get_metadata(self, meta_key):
        ...
        for dv in self.ds_views:
            ...
            if label is not None and lbl != label:
                raise ValueError(
                    'Mismatching metadata labels: %r != %r' % (label, lbl))
            label = lbl
        ...
        for dv in self.ds_views:
            ...
            if wave is None:
                ...
                if wave.shape != w.shape or not np.allclose(wave, w):
                    raise ValueError(
                        "Mismatching wavelength data in %s." % dv.ds)
            X.append(x)
