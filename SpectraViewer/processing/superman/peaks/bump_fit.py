...
class BumpFit(PeakFinder):
    ...
    def _fit_one(self, bands, intensities):
        max_peaks = self._max_peaks if self._max_peaks is not None else len(
            bands)
...
def fit_single_peak(bands, intensities, loc, fit_kind='lorentzian',
    ...
    params, pstd = _weighted_curve_fit(bands, intensities, loc, fit_func,
                                       params,
...
def fit_composite_peak(bands, intensities, locs, num_peaks=2, max_iter=10,
    ...
    if len(locs) == num_peaks:
        ...
        loc_guesses = np.linspace(locs[0] - band_resolution,
                                  locs[0] + band_resolution,
                                  num_peaks)
    else:
    ...
    for fit_keys in combinations_with_replacement(fit_funcs, num_peaks):
        ...
        combs.append((residual, fit_keys, fit_func, params, pstd,
                      mask, peak_x, peak_y))
    residual, fit_keys, fit_func, params, pstd, mask, peak_x, peak_y = min(
        combs)
...
def _weighted_curve_fit(bands, intensities, loc, fit_func, params,
    ...
    for i in xrange(max_iter):
        ...
        try:
            params, pcov = curve_fit(fit_func, bands, intensities, p0=params,
                                     sigma=w)
        ...
        log_fn(
            '_weighted_curve_fit failed to converge in %d iterations' % max_iter)
...
try:
    scipy.optimize.curve_fit(lambda x, a: x + a, [0, 1], [0, 1], p0=(0,),
                             bounds=(0, 1))
except TypeError:
