...
class BaseMLP(BaseEstimator):
    ...
    def fit(self, X, y, max_epochs, shuffle_data, verbose=0):
        ...
        for i, batch_slice in enumerate(
                islice(cycle(batch_slices), n_iterations)):
            self._forward(i, X, batch_slice, x_hidden, x_output)
            self._backward(i, X, y, batch_slice, x_hidden, x_output, delta_o,
                           delta_h)
        return self
    ...
    def _backward(self, i, X, y, batch_slice, x_hidden, x_output, delta_o,
        ...
        # update weights
        self.weights2_ += self.lr / self.batch_size * np.dot(x_hidden.T,
                                                             delta_o)
        self.bias2_ += self.lr * np.mean(delta_o, axis=0)
        self.weights1_ += self.lr / self.batch_size * X[batch_slice].T.dot(
            delta_h)
